...
class SecurityConfig {
    ...
    @Bean
    fun securityFilterChain(http: HttpSecurity, jwtAuthFilter: JwtAuthFilter, @Value("\${cors.origin}") origin: String): SecurityFilterChain? {
        http
            .authorizeHttpRequests { auth -> auth
                ...
                .requestMatchers(HttpMethod.PATCH, "/api/v1/users") // for changing name
                .hasRole(UserRole.ROLE_USER.withoutPrefix)
                ...
                .requestMatchers("/api/v1/specialties/**", "/api/v1/groups/**", "/api/v1/users/**", "/api/v1/statistic/**")
                .hasRole(UserRole.ROLE_ADMIN.withoutPrefix)
                .anyRequest().authenticated()
            }
            .sessionManagement { session -> session.sessionCreationPolicy(SessionCreationPolicy.STATELESS) }
            .csrf { csrf -> csrf.disable() }
            .cors { cors -> cors.configurationSource(corsConfigurationSource(origin)) }
